...
在现有航运信息平台基础上扩展用户认证、舱位购买和订单管理功能。采用 Vue 3 + TypeScript 技术栈，使用 sessionStorage 管理登录状态，JSON 文件持久化用户和船期数据，浏览器localStorage持久化订单数据。新增登录页面、购买确认弹窗、订单查询页面，并在船期列表中集成库存展示和购买按钮。
